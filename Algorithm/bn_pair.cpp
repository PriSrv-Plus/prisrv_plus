...
Big PFC::Zpinverse(Big &x)//update
{
    Big z;
...
/////
Big PFC::Zpadd_mod(Big &a, Big &b)
{
    return (a+b)%(*mod);

}
Big PFC::Zpmulti_mod(Big &a, Big &b)
{
    return (a*b)%*mod;
 //   return (z)%(*ord);
}
Big PFC::Zpsub_mod(Big &a, Big &b)
{
    if(a>b)
        return a-b;
    else
...
Big PFC::Zpinverse_mod(Big &x)//update
{
    Big z;
    xgcd(x.fn,(*mod).fn,z.fn,z.fn,z.fn);
    return z;
}
///
...
PFC::PFC(int s, csprng *rng)
{
    ...
    if (s==128)	mod_bits=256*2;//256;
    if (s==192) mod_bits=768*2;//768;
...
void PFC::random_ord(Big& w)
{
    random(w);
    w=w%*ord;
}
void PFC::random_mod(Big& w)
{
    random(w);
    w=w%*mod;
}
